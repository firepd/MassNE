...
class HeadcountEffect(nn.Module):  # Marginal effect
    ...
    def forward(self, unit_nums):  # output utility
        if self.mode == 0:
...
class MassNE(nn.Module):
    def __init__(self, n_unit, hidden_dim=10, indi=True, coop=True, supp=True, need_att=True, device='cpu'):
        super(MassNE, self).__init__()
        assert n_unit > 1
        self.n_unit = n_unit
        self.hidden_dim = hidden_dim
        self.device = device
        self.cooperate = Cooperation(n_unit, hidden_dim, indi=indi, coop=coop, need_att=need_att)
        self.compete = Suppression(n_unit, hidden_dim, need_att=need_att)
        self.supp = supp
...
if __name__ == '__main__':
    ...
    model = MassNE(n_unit, 20, need_att=False)
    preds = model(match)
    print(preds.shape)
